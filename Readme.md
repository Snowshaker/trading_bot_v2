...
*   **💬 Управление через Telegram:**
    ...
    *Приветственное сообщение и список команд (`/start`):*
    ![Start Command](.images/start.png)

    *Просмотр баланса портфеля (`/balance`):*
    ![Balance Command](.images/balance.png)

    *Просмотр истории сделок (`/history`):*
    ![History Command](.images/history.png)

    *Просмотр последних N сделок (`/history 5`):*
    ![History N Command](.images/history_x.png)

    *Процесс продажи актива (`/sell`). Процесс покупки (`/buy`) актива происходит аналогичным образом:*
    *Шаг 1: Выбор торговой пары для продажи*
    ![Sell Step 1](.images/sell_1.png)
    *Шаг 2: Ввод количества и начало обработки*
    ![Sell Step 2](.images/sell_2.png)
    *Шаг 3: Подтверждение операции продажи*
    ![Sell Step 3](.images/sell_3.png)
    *Шаг 4: Успешное исполнение ордера и обновление балансов*
    ![Sell Step 4](.images/sell_4.png)
...
*   `.coveragerc`: Файл конфигурации для отчетов о покрытии тестами.
*   `Testing.txt`: Краткая информация о запуске тестов (содержимое этого файла интегрировано в данный README).

## 🧪 Тестирование

Проект покрыт набором модульных тестов для обеспечения стабильности и корректности работы ключевых компонентов. Тестирование выполняется с использованием фреймворка `pytest` совместно с `pytest-cov` для генерации отчетов о покрытии кода.

*   **Запуск тестов и генерация отчета о покрытии:**
    ```bash
    pytest --cov
    ```
*   **Конфигурация покрытия:** Настройки для `pytest-cov` (например, исключение определенных файлов или директорий из отчета) находятся в файле `.coveragerc`.
*   **Текущее состояние (на основе предоставленных данных):**
    *   **Собрано и запущено:** 189 тестов.
    *   **Общее покрытие кода:** 66%.

Мы стремимся к постоянному улучшению качества кода и увеличению тестового покрытия для всех критически важных модулей. Детальный отчет о покрытии по каждому файлу доступен в консоли после выполнения команды запуска тестов.
