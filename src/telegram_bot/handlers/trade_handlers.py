...
class TradeHandlers:
  ...
  async def confirm_trade(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
      ...
      quantity_decimal = context.user_data.get('quantity')
      ...
      if result and result.get('success'):
        ...
        try:
          balance_base = self.info_fetcher.get_asset_balance(base_asset)
      ...
      logger.error(f"Trade error (OrderExecutionError) for {context.user_data.get('symbol', 'N/A')}: {str(e)}",
                   exc_info=True)
      await query.edit_message_text(f"❌ Ошибка исполнения ордера: {str(e)}")
    except KeyError as e:
      logger.error(f"Trade error - KeyError: {str(e)} for symbol {context.user_data.get('symbol', 'N/A')}",
                   exc_info=True)
  ...
  def _has_balance_for_symbol(self, symbol: str, action: str) -> bool:
    ...
    try:
      ...
      if asset_to_check:
        balance_info = self.info_fetcher.get_asset_balance(asset_to_check)
        if balance_info and isinstance(balance_info.get('free'), Decimal):
          return balance_info['free'] > Decimal('0')
        logger.warning(
          f"Не удалось получить баланс или 'free' не Decimal для {asset_to_check} (символ {symbol}). balance_info: {balance_info}")
        return False
      else:
      ...
      logger.error(
        f"Ошибка при проверке баланса для действия '{action}' по символу {symbol} (актив: {asset_to_check}): {e}",
        exc_info=True)
      return False
